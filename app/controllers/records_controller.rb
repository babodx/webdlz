class RecordsController < ApplicationController
  ...
  #load_and_authorize_resource :class => RecordsController
  def show
  end

  def edit
    @user = current_user
    @zone = @user.zonenames.find(params[:name_id])
  ...
  def create
    @zone = Zonename.find(params[:name_id])
  ...
  def disable
    @user = current_user
    @zone = @user.zonenames.find(params[:name_id])
    @record = @zone.records.find(params[:record_id])
  ...
  def activate
    @user = current_user
    @zone = @user.zonenames.find(params[:name_id])
    @record = @zone.disabled_records.find(params[:record_id])
  ...
  def destroy_disabled
    @user = current_user
    @zone = @user.zonenames.find(params[:name_id])
    rec = @zone.disabled_records.find(params[:disabled_record_id])
    if rec.destroy
